...
EGVAR(end_conditions,playerCasLimit) = 100;  // Coop: Percentage of the max. allowed player casualty
...
EGVAR(end_conditions,taskLimit) = 8;  // Minimum number of completed tasks
...
EGVAR(end_conditions,extTask) = "task11";  // Extraction task that will be automatically completed (empty string disables this functionality)
EGVAR(end_conditions,playerThreshold) = 10;  // Percentage of the alive players that have to be in the extraction zone
...
EGVAR(respawn,timer) = 60;  // Respawn timer in seconds
...
GVARMAIN(serverViewDistance) = 750;  // Server view distance (= AI view distance)
...
EGVAR(ai_skills,skillSet) = [
    ...
    0.3,  // Commanding (Higher = Better)
    0.3,  // Courage (Higher = Better)
    0.2,  // Aiming Accuracy (Higher = Better)
    0.2,  // Aiming Shake (Higher = Less)
    0.2,  // Aiming Speed (Higher = Faster)
    0.2,  // Reload Speed (Higher = Faster)
    0.2,  // Spotting Distance (Higher = Further)
    0.2   // Spotting Time (Higher = Faster)
];
...
EGVAR(intro_text,title) = "MACV-SOG The Beginning";  // Title
EGVAR(intro_text,date) = "January 24, 1964";  // Date
EGVAR(intro_text,location) = "CCN: FOB 3 - Khe Sanh Special Forces Camp";  // Location
EGVAR(intro_text,delay) = 15;  // Delay after loading in


// JIP
GVARMAIN(moduleJIP) = true;  // Coop & TvT
EGVAR(jip,jipTimer) = 15 MINUTES;  // For how long the TP is available after joining the mission
...
EGVAR(map_cover,aoMarker) = "mrk_ao_1";  // Name of the AO marker
...
EGVAR(reinsertion,markRP) = false;  // Mark the RP location on map
EGVAR(reinsertion,RPObject) = "Land_vn_tenta_F";  // Object used as the squad rally point
EGVAR(reinsertion,markHAB) = false;  // Mark the HAB location on map
EGVAR(reinsertion,HABObject) = "Land_vn_tent_02_01";  // Object used as the platoon HAB
EGVAR(reinsertion,allowTPFromHAB) = false;  // If TP to squad RP is allowed from the platoon HAB
...
EGVAR(supply_drop,supplyDropPlane) = "vn_i_air_ch34_02_01";  // Plane or helicopter used for the supply drop
