...
_bandits = [];
_groupConfig = configfile >> "CfgGroups" >> "East" >> "UK3CB_CCM_O" >> "Infantry" >> "UK3CB_CCM_O_RIF_Squad";
...
_paraBandits = [];
_groupConfig = configfile >> "CfgGroups" >> "Indep" >> "rhsgref_faction_nationalist" >> "rhsgref_group_national_infantry" >> "rhsgref_group_national_infantry_squad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=((_groupConfig) select _x);
    if (isClass _item) then {
...
_eastSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "East" >> "rhs_faction_msv" >> "rhs_group_rus_msv_infantry" >> "rhs_group_rus_msv_infantry_squad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=((_groupConfig) select _x);
    if (isClass _item) then {
...
_indSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "Indep" >> "rhssaf_faction_army" >> "rhssaf_group_army_m10_digital_infantry" >> "rhssaf_group_army_m10_digital_infantry_weaponsquad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=(_groupConfig select _x);
    if (isClass _item) then {
...
_natoSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "West" >> "rhs_faction_usmc_wd" >> "rhs_group_nato_usmc_wd_infantry" >> "rhs_group_nato_usmc_wd_infantry_squad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=(_groupConfig select _x);
    if (isClass _item) then {
