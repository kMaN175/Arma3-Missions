...
for "_x" from 0 to (_count-1) do {
	_weap = ((configFile >> "CfgWeapons") select _x);
		if (isClass _weap) then {
			_libCheck = (configname _weap select [0,4]);
			_libCheckSpecial = (configname _weap select [2,4]);
			if ((_libCheck == "LIB_") or (_libCheckSpecial == "LIB_")) then {
...
for "_x" from 0 to (_count-1) do {
    ...
    if (isClass _item) then {
			_libCheck = (configname _item select [0,6]);
				if (_libCheck == "B_LIB_") then {
...
for "_x" from 0 to (_count-1) do {
    _item=((configFile >> "CfgFaces" >> "Man_A3") select _x);
    		_libCheck = (configname _item select [0,4]);
    if (isClass _item && _libCheck == "LIB_") then {_faces = _faces + [configName _item];};
};
...
List_Items = [] + _items + ['ItemCompass','ItemMap','LIB_ACC_GW_SB_Empty','LIB_ACC_GL_M7','LIB_ACC_GL_Enfield_CUP_Empty','LIB_ACC_GL_DYAKONOV_Empty']; //'ItemGPS', 'ItemWatch', 'ItemRadio',
List_Mines = [] + _mines;
List_Glasses = [] + _primaries;
List_Faces = [] + _secondaries;
