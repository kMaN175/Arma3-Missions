...
if (specialWave && attkWave >= 10) then {
	...
	switch (_randWave) do
	{
		...
		case 5:
		{
			SpecialWaveType = "defectorWave";		//defectorWave
		};
		case 6:
		{
			SpecialWaveType = "PTRDWave";
		...
		case 7:
		{
			SpecialWaveType = "grenadierWave";
		};
		case 8:
		{
			SpecialWaveType = "flameWave";
		};
		case 9:
		{
			SpecialWaveType = "MGWave";
		};
		//case 7:
		//{
		//	SpecialWaveType = "demineWave";		//demineWave
		//};
	};
	wavesSinceSpecial = 0;
//}else{
//	SpecialWaveType = "PTRDWave"; //else for testing new special waves: do not remove
};
...
if (SpecialWaveType == "flameWave") then {
	flameWave = true;
}else{
	flameWave = false;
};
...
if (SpecialWaveType == "PTRDWave") then {
	PTRDWave = true;
}else{
	PTRDWave = false;
};

if (SpecialWaveType == "grenadierWave") then {
	grenadierWave = true;
}else{
	grenadierWave = false;
};
...
if (defectorWave) then {
	["SpecialWarning",["US Army Troops Are Attacking Us!"]] remoteExec ["BIS_fnc_showNotification", 0];
	["Alarm"] remoteExec ["playSound", 0];
};
...
if (MGWave) then {
	["SpecialWarning",["MG42 Wave incoming!"]] remoteExec ["BIS_fnc_showNotification", 0];
	["Alarm"] remoteExec ["playSound", 0];
};
...
if (grenadierWave) then {
	["SpecialWarning",["Grenadier Wave Incoming!"]] remoteExec ["BIS_fnc_showNotification", 0];
	["Alarm"] remoteExec ["playSound", 0];
};
