...
_germanSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "West" >> "LIB_WEHRMACHT" >> "Infantry" >> "LIB_GER_infantry_squad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=(_groupConfig select _x);
    if (isClass _item) then {
		_germanSoldier pushback getText (_item >> "vehicle");
    };
};
List_Wehrmacht = _germanSoldier;

_usSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "Indep" >> "LIB_US_ARMY" >> "Infantry" >> "LIB_US_AT_Squad";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=(_groupConfig select _x);
    if (isClass _item) then {
		_usSoldier pushback getText (_item >> "vehicle");
    };
};
List_USArmy = _usSoldier;

_gerRifleSoldier = [];
_groupConfig = configfile >> "CfgGroups" >> "West" >> "LIB_WEHRMACHT" >> "Infantry" >> "LIB_GER_sentry_squad_3";
_count = count (_groupConfig);
for "_x" from 0 to (_count-1) do {
    _item=(_groupConfig select _x);
    if (isClass _item) then {
		_gerRifleSoldier pushback getText (_item >> "vehicle");
    };
};
List_WehrmachtRifle = _gerRifleSoldier;
...
for "_x" from 0 to (_realentries) do {
  ...
  if (isClass _checked_veh) then { // CHECK IF THE SELECTED ENTRY IS A CLASS
	...
	_libCheck = (configname _checked_veh select [0,4]);
    if (_vehclass == _vehClass && _scope != 0 && _simulation_paracheck != "parachute" && _classname != "LIB_US6_BM13" && _classname != "LIB_US6_BM13_SingleFire" && _actual_vehclass == "Armored" && _libCheck == "LIB_") exitWith {
...
for "_x" from 0 to (_realentries) do {
  ...
  _libCheck = (configname _checked_veh select [0,4]);
  _classname = configName _checked_veh;
	if (isClass _checked_veh  && _libCheck == "LIB_" ) then {
		_vehclass = getText (_checked_veh >> "vehicleClass");
		_scope = getNumber (_checked_veh >> "scope");
		_simulation_paracheck = getText (_checked_veh >> "simulation");
		_actual_vehclass = getText (_checked_veh >> "vehicleClass");
...
List_PTRD = ["LIB_SOV_scout_atrifle_gunner"];
List_Grenadier = ["LIB_GER_ober_grenadier"];
List_MG = ["LIB_GER_mgunner"];