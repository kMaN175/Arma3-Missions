...
if (PTRDWave) then { //determine if PTRD wave and spawn from List defined in EditMe.sqf
	unitClasses = PTRD_CLASS;
};

if (grenadierWave) then { //determine if Grenadier wave and spawn from List defined in EditMe.sqf
	unitClasses = GRENADIER_CLASS;
};

if (MGWave) then { //determine if MG wave and spawn from List defined in EditMe.sqf
	unitClasses = MG_CLASS;
};
//excluding special waves with custom unit classes
if !(defectorWave || MGWave || PTRDWave || grenadierWave) then {
...
for ("_i") from 1 to _unitCount do {
	...
	if(_attackWave <= PISTOL_HOSTILES) then {
		...
		_unit addMagazine "LIB_8Rnd_9x19";
		_unit addMagazine "LIB_8Rnd_9x19";
		_unit addWeapon "LIB_P38";
...
//flamethrower wave
	if (flameWave) then {
		...
		// "Add containers";
		_unit forceAddUniform "U_LIB_US_NAC_Uniform";
