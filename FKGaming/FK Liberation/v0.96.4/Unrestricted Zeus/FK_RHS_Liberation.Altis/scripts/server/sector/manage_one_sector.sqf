...
private _sector_despawn_tickets = 50;
...
if ((!(_sector in blufor_sectors)) && (([getmarkerpos _sector, [_opforcount] call F_getCorrectedSectorRange, GRLIB_side_friendly] call F_getUnitsCount) > 0)) then {
	...
	if (_sector in sectors_bigtown) then {	
		...
		if (KP_liberation_civ_rep < 45) then {
			_iedcountX = round (1 + (ceil (random 5)) * (round ((KP_liberation_civ_rep * -1) / 50)) * GRLIB_difficulty_modifier/100);
			_iedcount = abs(_iedcountX);
		};
		if (_iedcount > 13) then {_iedcount = 13};
	};
	...
	if (_sector in sectors_capture) then {
		...
		if (_iedcount > 2) then {_iedcount = 2};
	};
