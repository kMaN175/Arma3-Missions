...
if ((Alive _Heli) && (Alive (driver _Heli))) then 
{
	...
	WaitUntil {sleep 1; (_Heli emptyPositions "CARGO" == 0) or (!Alive _Heli or !Alive (driver _Heli) or !CanMove _Heli) or ({(isPlayer _x) && !(Vehicle _x in Vehicles) && (Alive _x)} Count (NearestObjects [_Heli, ["CAMANBase"], 300]) == 0) or (((GetPosATL _Heli) Select 2) > 15)};
...
if ({isPlayer _x} count (Crew _Heli) > 0) then
{
	...
	if ((Alive _Heli) && (Alive (driver _Heli))) then 
	{
		...
		{if (isPlayer _x && Alive _x) then {_x action ["getOut", _Heli]} } forEach (crew _Heli);
		
		WaitUntil {sleep 1; ( {isPlayer _x} count (Crew _Heli) == 0) or (!Alive _Heli or !Alive (driver _Heli) or !CanMove _Heli)};
...
[_Crew,_Heli,_STTD] spawn
{
	...
	WaitUntil {sleep 1; (!(Alive _Heli) or ({Alive _x} count (units _Crew) == 0) or (_Heli distance (_STTD select 3) <= 100))};
